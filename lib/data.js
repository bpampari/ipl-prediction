...
import { formatDateTimeLabel, formatJoinedAt } from "@/lib/format";
...
export async function getDashboardData(userId) {
  ...
  const [{ data: membership }, { data: memberRows }, { data: profiles }, { data: matches }, { data: currentProfile }] =
    await Promise.all([
      ...
      supabase
        .from("matches")
        .select("id, room_id, team_a, team_b, match_date, match_time, actual_winner, settled_at, created_at")
        .eq("room_id", room.id)
  ...
  const matchesWithLabels = (matches || []).map((match) => ({
    ...match,
    match_date_label: formatDateTimeLabel(match.match_date, match.match_time),
    predictions_locked: new Date(`${match.match_date}T${match.match_time}+05:30`).getTime() <= Date.now()
  }));
